#include "doctest.h"
#include "tux/view32.hpp"
#include "tux/util.hpp"
#include "views32.h"

using namespace std;
using namespace tux;
template<typename T> using view = view32<T>;

TEST_SUITE("view32");

TEST_CASE("view32 size by name")
{
    CHECK(view_size32("my_struct") == sizeof(my_struct));
    CHECK(view_size32("string_info") == sizeof(string_info));
    CHECK(view_size32(string("string_info")) == sizeof(string_info));
}
...
TEST_CASE("view32 default constructor")
{
    view<string_info> v;
...
TEST_CASE("view32 copy constructor")
{
    view<string_info> v1;
    v1->ascii_sum = 3;
    v1->byte_count = 10;
...
TEST_CASE("view32 move constructor")
{
    view<string_info> v1;
    v1->ascii_sum = 3;
    v1->byte_count = 10;
...
TEST_CASE("view32 move construct from buffer")
{
    view<string_info> v1;
    v1->ascii_sum = 3;
    v1->byte_count = 10;
...
TEST_CASE("view32 move assign from buffer")
{
    view<string_info> v1;
    v1->ascii_sum = 3;
    v1->byte_count = 10;
...
TEST_CASE("view32 manual alloc and free")
{
    view32<string_info> v;
    CHECK(v.buffer().data() == nullptr);
    v.alloc();
    CHECK(v.buffer().data() != nullptr);
    CHECK(v.buffer().data_size() == sizeof(string_info));
    CHECK(v.buffer().type() == "VIEW32");
...
TEST_CASE("view32::boolean_expression")
{
...
TEST_CASE("view32::arithmetic_expression")
{
